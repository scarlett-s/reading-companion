...
app/                    → expo-router 页面（文件即路由）
  _layout.tsx           → 根布局
  (tabs)/               → 底部标签页
    index.tsx           → 记录（首页）：最近读的 3-4 本书（按最近记录时间排序）+「添加图书」「全部图书」入口
    calendar.tsx        → 日历视图（每日阅读记录）
    settings.tsx        → 设置（AI 配置）
  book/list.tsx         → 全部图书（完整书架）
  book/search.tsx       → 搜索添加图书（书名 → 结果带封面，支持手动录入）
  book/[id].tsx         → 图书详情（历史记录 + 整理入口）
  entry/new.tsx         → 记录当日进度（页数/百分比）+ 评论（选模式）
  entry/discuss.tsx     → Discuss 对话页
src/                    → 业务逻辑（与 UI 解耦）
  types.ts              → 类型定义
...
3. Discuss 模式：AI 基于评论提问 → 最多 3 轮 → 保存「评论 + 对话 + 提炼要点」。
4. 整理：一键生成对该书的结构化思考整理，可重复、可保存。
5. 设置里填 DeepSeek Key 可用；切换 Ollama 后离线可用。
6. 记录/阅读无需网络；只有 Discuss / 整理需要 AI。
7. Mac 浏览器 + iPhone Expo Go 均可运行。
